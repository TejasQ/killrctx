  ...
  async function bulkDelete() {
    ...
    try {
      for (const podcastId of selected) {
        const res = await fetch(`/api/notebooks/${notebookId}/podcasts/${podcastId}`, {
          method: "DELETE",
        });
        if (!res.ok) failed.push(podcastId);
      }
      if (failed.length > 0) {
        setDeleteError(`${failed.length} episode${failed.length > 1 ? "s" : ""} could not be deleted.`);
      }
    } finally {
      setSelected(new Set());
      setDeleting(false);
